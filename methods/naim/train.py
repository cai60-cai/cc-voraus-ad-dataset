...
from .config_naim import NAIMConfig
from .model import NAIMModel


def build_dataloaders(config: NAIMConfig):
...
def main() -> None:
    ...
    training_cfg = TrainingConfig(learning_rate=5e-4)
    config = NAIMConfig(dataset=dataset_cfg, training=training_cfg)

    seed_everything(config.training.seed)
    model = NAIMModel(config)
